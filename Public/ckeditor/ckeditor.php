...
class plgEditorCKeditor extends JPlugin
{
	...
	function onDisplay( $name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null)
	{
		...
		$linkBrowserUrl
		CKEDITOR.config.extraPlugins = \"".implode(',',$pluginsName).$autogrow.$tableresize.$customConfigPlugins."\";
		$pluginsPath
		$baseHref
		...
		if($this->params->get( 'ckfinder','1') == 1)
		{
			...
			//$access_true = true;
			//$sessionActive = true;
			//$session->set('CKFinderAccess',true); //default false - user can't use CKFinder
